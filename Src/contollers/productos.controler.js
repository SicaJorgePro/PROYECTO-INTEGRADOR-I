...
// ! funcion de mostrar productos por identificacion
...
const productos_id = async (req,res) => {
  ...
  try {
    ...
    res.status(200).render("listado_producto", { titulo: "PRODUCTOS", producto });
    return;
     
  } catch (error) {

    if (error.status==404){
      res.status(error.status).render("error_404", { titulo: error.message });
    }else{
      res.status(error.status).render("error", { titulo: error.message });
    }
   }
...
// ! funcion de mostrar productos por categoria
